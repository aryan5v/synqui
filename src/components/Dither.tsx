...
void main() {
  ...
  // Create cool gradient colors
  vec3 color1 = vec3(0.2, 0.1, 0.8);  // Deep blue
  vec3 color2 = vec3(0.8, 0.2, 0.6);  // Magenta
  vec3 color3 = vec3(0.1, 0.6, 0.8);  // Cyan
  vec3 color4 = vec3(0.6, 0.1, 0.9);  // Purple
  
  // Use position to create gradient zones
  float gradientX = (uv.x + 1.0) * 0.5; // 0 to 1
  float gradientY = (uv.y + 1.0) * 0.5; // 0 to 1
  
  // Mix colors based on position and pattern
  vec3 topMix = mix(color1, color2, gradientX);
  vec3 bottomMix = mix(color3, color4, gradientX);
  vec3 baseColor = mix(bottomMix, topMix, gradientY);
  
  vec3 col = mix(vec3(0.0), baseColor, f * 1.2);
...
// Get time-based color that shifts throughout the day
function getTimeBasedColor(): [number, number, number] {
    ...
    if (hour >= 6 && hour < 12) {
        ...
        // Afternoon: warm teals and greens
        return [0.3, 0.7, 0.6];
    } else if (hour >= 18 && hour < 22) {
        // Evening: oranges and magentas
        return [0.8, 0.4, 0.6];
    } else {
...
export default function Dither({
    ...
    // Update colors every minute (no mouse interaction)
    useEffect(() => {
    ...
    return (
        ...
        >
            <DitheredWaves
                ...
                waveColor={dynamicColors ? currentColor : waveColor}
                colorNum={colorNum}
                pixelSize={pixelSize}
